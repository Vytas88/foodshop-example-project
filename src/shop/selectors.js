...
export const getProducts = state => state[MODULE_NAME].products.data; //iskart grazina rezultata
...
export const getFavoriteProducts = state => {
  const favoriteProducts = getProducts(state).filter(product =>
    getProducts(state).includes(product.id)
  );
...
export const isProductFavorite = (state, id) =>
  getFavorites(state).includes(id);
